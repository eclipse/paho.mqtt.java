...
public class OfflineBufferingTest{
	...
	/**
	 * Tests that A message can be buffered whilst the client is in a disconnected state and is
	 * then delivered once the client has reconnected automatically.
	 */
	@Test
	public void testSingleMessageBufferAndDeliver() throws Exception {
		String methodName = Utility.getMethodName();
	    LoggingUtilities.banner(log, cclass, methodName);
	    ConnectionManipulationProxyServer proxy = new ConnectionManipulationProxyServer(serverURI.getHost(), serverURI.getPort(), 4242);
	    proxy.startProxy();
    	...
    	MqttAsyncClient client = new MqttAsyncClient("tcp://localhost:4242", methodName, DATA_STORE);
	...
	/**
	 * Tests that multiple messages can be buffered whilst the client is in a disconnected
	 * state and that they are all then delivered once the client has connected automatically.
	 */
	@Test
	...
	/**
	 * Tests that the buffer correctly handles messages being buffered when the buffer is full
	 * and deleteOldestBufferedMessage is set to true.
	 */
	@Test
	public void testDeleteOldestBufferedMessages() throws Exception {
    	...
    	// Make sure that the message now at index 0 in the buffer is '1' instead of '0'
	...
	/**
	 * Tests that if enabled, buffered messages are persisted to the persistence layer
	 */
	@Test
	public void testPersistBufferedMessages() throws Exception {
    	...
    	MqttAsyncClient client = new MqttAsyncClient("tcp://localhost:4242", methodName, persistence);
    	DisconnectedBufferOptions disconnectedOpts = new DisconnectedBufferOptions();
    	disconnectedOpts.setBufferEnabled(true);
    	client.setBufferOpts(disconnectedOpts);
	...
	/**
	 * Tests that persisted buffered messages are published correctly when the client
	 * connects for the first time and are un persisted.
	 */
	@Test
	public void testUnPersistBufferedMessagesOnNewClient() throws Exception {
    	...
    	// Create Subscription client to watch for the message being published as soon as the main client connects
