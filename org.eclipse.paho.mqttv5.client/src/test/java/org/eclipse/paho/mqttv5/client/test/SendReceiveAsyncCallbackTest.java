...
public class SendReceiveAsyncCallbackTest {
	...
	class onConnect implements IMqttActionListener {
		...
		public void onSuccess(IMqttToken token) {
			...
			try {
				if (testno == 1) {
					token.getClient().subscribe(new MqttSubscription(topicFilter, 2), 
							 null, new onSubscribe(1), myListener, null);
				} else {
					Assert.fail("Wrong test numnber:" + methodName);
					testFinished = true;
				}
	...
	public void test1() throws Exception {
		...
		try {
			...
			mqttClient.disconnect(30000, null, new onDisconnect(1), 0, new MqttProperties());
