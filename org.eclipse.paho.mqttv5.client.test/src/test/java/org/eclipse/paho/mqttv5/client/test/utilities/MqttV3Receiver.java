...
public class MqttV3Receiver implements MqttCallback {
	 ...
	 * payload>"
	 * 
	 * We want to detect excess messages, so we don't just handle a certain number.
	 * Instead we wait for a timeout period, and exit if no message is received in
	 * that period.<b> The timeout period can make this test long running, so we
	 * attempt to dynamically adjust, allowing 10 seconds for the first message and
	 * then averaging the time taken to receive messages and applying some fudge
	 * factors.
	 * 
	 * @param sendTopics
