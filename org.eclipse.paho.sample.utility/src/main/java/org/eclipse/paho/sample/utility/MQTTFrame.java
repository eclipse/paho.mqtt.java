...
public class MQTTFrame implements ActionListener, MqttCallback, Runnable {
        ...
        if (connected) {
            try {
                ...
                JOptionPane.showMessageDialog(frame, ex.getClass().getName() + "\n" + ex.getMessage(),
                        "MQTT Publish Exception", JOptionPane.ERROR_MESSAGE);
                throw ex;
            }
        } else {
    ...
    /*A wrapper for the MQTT connect method. If the ip address, port number or persistence flag has changed since the last time then a new MqttClient object is required. If these values haven't changed then any previously created object can be used. Check whether Last Will & Testament is required and call the appropriate connect method. The only persistence implementation supported at this moment is MqttFilePersistence. *
    * */
    /**
     ...
     * <UL><LI>Connect - Check the client is not already connected, set the LED colour to amber to indicate connect
     * is in progress and start a thread to do the connect.
